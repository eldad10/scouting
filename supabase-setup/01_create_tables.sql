...
-- Create Forms table
-- auto_balls and teleop_balls are now INT (exact count entered by scout, not a range)
CREATE TABLE IF NOT EXISTS forms (
  scoutername        VARCHAR(100) NOT NULL,
  matchnumber        INT          NOT NULL,
  teamnumber         VARCHAR(100) NOT NULL,
  auto_balls         INT          DEFAULT 0,
  auto_climb         BOOLEAN      DEFAULT false,
  auto_labels        TEXT         DEFAULT '',
  teleop_balls       INT          DEFAULT 0,
...
-- Robot scouting card per team (static robot info)
...
CREATE INDEX IF NOT EXISTS idx_forms_teamnumber  ON forms(teamnumber);
CREATE INDEX IF NOT EXISTS idx_forms_matchnumber ON forms(matchnumber);
CREATE INDEX IF NOT EXISTS idx_forms_scoutername ON forms(scoutername);
